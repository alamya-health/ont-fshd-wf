...
process BUILD_FSHD_REPORT {
  ...
  script:
    ...
    python3 - "${sample_id}" "${classification_dir}" "${subset_dir}" "${flagstat_txt}" "${coverage_tsv}" "${haplotag_summary_tsv}" "${methylation_summary_tsv}" "${variant_summary_tsv}" "${params.contraction_threshold_ru}" "${sample_id}.fshd.report.html" "${sample_id}.fshd.report.summary.tsv" <<'PY'
import csv
import html
import os
import sys
...
sample_id, classification_dir, subset_dir, flagstat_txt, coverage_tsv, hap_tsv, methyl_tsv, variant_tsv, contraction_threshold, html_out, summary_out = sys.argv[1:]
...
def metric_card(label, value, note=""):
    return f"<div class='metric'><div class='metric-label'>{html.escape(label)}</div><div class='metric-value'>{html.escape(str(value))}</div><div class='metric-note'>{html.escape(note)}</div></div>"
...
def svg_hist(values, color, label):
    ...
    for idx, x in enumerate(xs):
        ...
        ypos = 170 - bar_h
        bars.append(f"<rect x='{xpos}' y='{ypos}' width='{bar_width}' height='{bar_h}' rx='8' fill='{color}' opacity='0.88' />")
        bars.append(f"<text x='{xpos + bar_width / 2}' y='190' text-anchor='middle' class='axis'>{x}</text>")
        bars.append(f"<text x='{xpos + bar_width / 2}' y='{max(24, ypos - 8)}' text-anchor='middle' class='count'>{count}</text>")
...
data = {name: read_semicolon_csv(os.path.join(classification_dir, filename)) for name, filename in category_files.items()}
...
if contracted:
    headline = f"Complete permissive 4qA reads with <= {contraction_threshold} repeat units were observed."
elif shortest_4qa is not None:
    headline = f"Complete 4qA reads were observed, but the shortest measured array exceeded {contraction_threshold} repeat units."
...
pas_counter = Counter(row.get("PAS.type", "NA") for row in data["4qA_complete"] if row.get("PAS.type"))
top_4qa_rows = sorted(data["4qA_complete"], key=lambda row: (to_int(row.get("RU_count"), 10**9), row.get("read.id", "")))[:15]
top_chimeric_rows = sorted(data["chimeric"], key=lambda row: (to_int(row.get("RU_count"), 10**9), row.get("read.id", "")))[:12]
