...
process SUMMARIZE_COHORT {
  ...
  script:
    ...
    python3 - "cohort.fshd.summary.tsv" "cohort.fshd.summary.html" ${summary_files} <<'PY'
import csv
import html
import sys
...
contracted_count = sum(1 for row in rows if row.get("contracted_4qA_observed", "").lower() == "true")
